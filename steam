...
# line 165 was added to create an empty file so muselsl knows when to start/stop streaming (if the file "Stop_EEG1.csv" exists, then we keep streaming)
# lines 176-182 constantly checks to see if the above file exists. If it doesn't, streaming stops and the muse is disconnected
    if(didConnect):
        print('Connected.')
        stop_eeg = [0]
        np.savetxt("/Users/mathlab/muse_exp/Experiments/Stop_EEG1.csv", (stop_eeg), delimiter=',',fmt="%s")
        muse.start()
        ...
        while os.path.isfile("/Users/mathlab/muse_exp/Experiments/Stop_EEG1.csv") == True:
            try:
                sleep(1)
